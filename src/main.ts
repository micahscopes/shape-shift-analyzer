...
let showTwoOctaves: boolean = false;
...
eventSystem.registerEffectHandler("LOAD_SCENE_AUDIO", (effect, state) => {
  if (effect.type === "LOAD_SCENE_AUDIO") {
    ...
    if (scene) {
      ...
      console.log(
        `Starting new audio request ${requestId} for scene ${effect.sceneIndex}`,
      );
...
function createSceneHeader(scene: Scene, state: AppState): HTMLElement {
  ...
  const modeToggleBtn = header.querySelector(
    "#mode-toggle",
  ...
  modeToggleBtn?.addEventListener("click", () =>
    eventSystem.dispatch({ type: "MODE_TOGGLE" }),
  );
...
function createSceneContent(scene: Scene, state: AppState): HTMLElement {
  ...
  if (scene.referenceClip) {
    ...
    setTimeout(() => {
      ...
      if (playPauseBtn) {
        playPauseBtn.addEventListener("click", () =>
          eventSystem.dispatch({ type: "PLAY_PAUSE" }),
        );
        updatePlayPauseButton(state);
      }
    }, 0);
    ...
    leftColumn.innerHTML =
      '<div class="no-reference">No reference clip for this scene</div>';
  }
  ...
  if (scene.shapeClip && scene.pitchClassSet) {
    ...
    rightColumn.innerHTML =
      '<div class="no-shape">No shape clip for this scene</div>';
  }
...
function createShapeAnalysis(scene: Scene): HTMLElement {
  ...
  const pitchClasses = Array.from(scene.pitchClassSet.pitchClasses).sort(
    (a, b) => a - b,
  );
  const pcNames = pitchClasses.map((pc) =>
    PitchClassSetAnalyzer.getPitchClassName(pc),
  );
  ...
  setTimeout(() => {
    const octaveToggle = document.getElementById(
      "octave-toggle",
    ) as HTMLButtonElement;
    if (octaveToggle) {
...
async function loadSceneAudio(
  ...
  isAudioLoading = true;
  console.log(
    `Loading audio for scene ${scene.index} with request ${requestId}`,
  );
      ...
      if (state.isAutoMode) {
        audioPlayer.setLoopMode("once", () => {
          ...
          if (
            ...
            console.log(
              "Skipping auto-advance - mode changed, not playing, or navigation in progress",
            );
          }
        });
      } else {
...
async function checkForLocalFile() {
  try {
    ...
    if (response.ok) {
      ...
      if (fileName) {
        fileName.textContent =
          "Joanna's Theme analysis.dawproject (auto-loaded)";
      }
    ...
    console.log(
      "No local DAWproject file found, waiting for user to select one.",
    );
  }
}
